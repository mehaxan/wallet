...
import { db } from "@/db";
import { netWorthSnapshots, accounts, loans, investments, assets, dpsAccounts } from "@/db/schema";
import { requireSession } from "@/lib/session";
import { apiError } from "@/lib/utils";

export const dynamic = "force-dynamic";

/** POST /api/networth/snapshot — saves a snapshot for the current month */
export async function POST(_req: NextRequest) {
  try {
    ...
    const [snap] = await db.insert(netWorthSnapshots).values({
      ...
      .onConflictDoUpdate({
        target: [netWorthSnapshots.userId, netWorthSnapshots.month, netWorthSnapshots.year],
        set: {
          totalAssets: totalAssets.toString(),
          totalLiabilities: totalLiabilities.toString(),
          netWorth: netWorth.toString(),
